...
export default function StanceFeed({ num_stances, entity, initialStanceId }: StanceFeedProps) {
    ...
    useEffect(() => {
        async function initialFetch() {
            try {
                setLoading(true);
                const response = await entityService.getStancesByEntity(API, entity);
                ...
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        ...
                        if (idx !== -1 && idx !== currentIndex) {
                            setCurrentIndex(idx);
                            window.history.replaceState(null, "", `/entities/${entity}/feed/${stances[idx].id}`);
                        }
                    }
                });
            },
    ...
    const fetchStances = useCallback(async () => {
        try {
            ...
            if (newStances.stances.length === 0) {
                setHasMore(false);
            } else {
                setPage(prev => prev + 1);
            }
