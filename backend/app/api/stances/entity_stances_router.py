...
def create_stance_endpoint(
    ...
    try:
        ...
        for url in image_urls:
            image_db.create_image(db, stance_id=stance_obj.id, entity_id=None, public_url=url, file_size=0, file_type="image/png")
...
def update_stance_endpoint(
    ...
    try:
        ...
        if not stance_obj:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Stance not found or not authorized")
...
def delete_stance_endpoint(
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to delete stance")
...
def rate_stance_endpoint(
    ...
    try:
        ...
        success: bool = rating_db.rate_stance(db, user_id=user_id, stance_id=stance.id, rating=request.rating)
