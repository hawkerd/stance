...
router = APIRouter(tags=["comments"], prefix="/entities/{entity_id}/stances/{stance_id}/comments")
...
def create_comment_endpoint(
    ...
    try:
        ...
        comment: Comment = comment_db.create_comment(db, user_id=user_id, stance_id=entity_stance[1].id, content=request.content, parent_id=request.parent_id)
...
def react_to_comment(
    ...
    try:
        ...
        if not existing_reaction:
            new_reaction: CommentReaction = comment_reaction_db.create_comment_reaction(db, current_user, comment.id, request.is_like)
            return
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update reaction")
