...
def read_user_followers(db: Session, user_id: int, cursor: datetime | None = None, limit: int | None = None) -> list[Follow]:
    try:
        query = db.query(Follow).filter(Follow.followed_id == user_id).order_by(Follow.created_at.desc())
        if cursor:
...
def read_user_following(db: Session, user_id: int, cursor: datetime | None = None, limit: int | None = None) -> list[Follow]:
    try:
